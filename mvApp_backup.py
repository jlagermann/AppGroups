...
ACCESS_CODE = ''
...
def validate_args(self):
    ...
    if args.restore and (args.collect or args.from_group or args.to_group or args.group_restore):
        print('Cannot specify restore with any other option.')
        exit(1)

    if args.collect and (args.restore or args.from_group or args.to_group or args.group_restore):
        print('Cannot specify collect with any other option.')
        exit(1)

    if args.group_restore and (args.restore or args.from_group or args.to_group or args.collect):
...
def do_put_data(data, headers, to_group_id):
    ...
    resp = requests.put(url, data=json.dumps(new_data), verify=False, headers=headers)
    # log the application that was moved to include the to and from group names and ID's
    logger.warning('Moved application ID:' + str(data['id']) + ' (' + str(app) + ') from group ID:' + str(from_group_id) + ' (' + application_group_id[str(from_group_id)] + ') to group ID:' + str(to_group_id) + ' (' + application_group_id[str(to_group_id)] + ')')


def collect_rest_api(access_token):
    """ Runs test for REST API
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--group_restore', help="Move a select group of applications to the default group", type=int)
    parser.add_argument('-r', '--restore', action="store_true", dest="restore", default=False, help="Move all applications to their default groups")
    parser.add_argument('-c', '--collect', action="store_true", dest="collect", default=False, help="Create a defaultGroups.txt file from a SCC")
    args = parser.parse_args()
    validate_args(args)
    from_group_id = args.from_group
    if args.to_group == None:
